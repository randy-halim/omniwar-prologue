...
package me.fcrh.omniwarprologue.world.dimension;
...
public class NullDemensionDimension extends OmniwarPrologueModElements.ModElement {
	public NullDemensionDimension(OmniwarPrologueModElements instance) {
		super(instance, 3);
	}
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				ObfuscationReflectionHelper.setPrivateValue(WorldCarver.class, WorldCarver.CAVE, new ImmutableSet.Builder<Block>()
						...
						.add(Blocks.BLACK_CONCRETE).build(), "field_222718_j");
	...
	public void clientLoad(FMLClientSetupEvent event) {
		DimensionRenderInfo customEffect = new DimensionRenderInfo(Float.NaN, true, DimensionRenderInfo.FogType.NONE, false, false) {
			...
			public Vector3d func_230494_a_(Vector3d color, float sunHeight) {
				return new Vector3d(0.580392156863, 0.580392156863, 0.580392156863);
			}
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("omniwar_prologue:null_demension"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
